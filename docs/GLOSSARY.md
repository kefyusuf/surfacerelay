...
A repo-local executable subject that claims one or more profiles/capabilities and provides process wiring for a harness. Target wiring is not canonical SurfaceRelay semantic truth and remains outside `spec/0.1` (D-061).

### Conformance observation

A raw fact returned by a target harness after executing one canonical scenario, such as whether execution returned/threw and how many framework/replacement dispatches occurred. Observations are not verdicts; the runner compares them to canonical expectations (D-059/D-060).

### Conformance verdict

The runner-owned result of comparing an applicable canonical expectation to a valid observation: `PASS` or `FAIL`. Infrastructure/protocol inability to obtain a trustworthy observation is `ERROR`; canonical capability selection may produce `NOT_APPLICABLE` before a harness is spawned. Harnesses do not self-report verdicts (D-059/D-060).
